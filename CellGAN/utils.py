...
class TrainingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        atac_sample = self.X_train[idx]
        gex_sample = self.y_train[idx]
        gex_sample = np.array(gex_sample)
        gex_sample = torch.from_numpy(gex_sample)
        #print("sample:", sample.shape)
        #sample = np.array_split(sample, 1)
        #print("sample2:", sample.shape)
        atac_sample = np.array(atac_sample)
        #df = pd.DataFrame(X_train, columns= ['Column_A'])
        atac_sample = torch.from_numpy(atac_sample)
        #print("sample3:", sample.shape)
        return atac_sample, gex_sample
